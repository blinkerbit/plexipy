...
class IsolatedBaseHandler(tornado.web.RequestHandler):
    ...
    def initialize(self, app_config: dict[str, Any] | None = None):
...
def authenticated(method):
    ...
    async def wrapper(self, *args, **kwargs):
        ...
        try:
            ...
            self.set_status(401)
            self.write({"error": f"Invalid token: {e!s}"})
            return None
...
class HealthHandler(IsolatedBaseHandler):
    ...
    async def get(self):
        ...
        self.success(data={"status": "healthy", "app": app_name, "isolated": True})
...
def main():
    ...
    try:
        ...
        logger.exception(f"Invalid port: {port_str}")
        sys.exit(1)
